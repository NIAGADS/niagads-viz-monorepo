...
export async function backendFetchFromRequest(request: NextRequest, apiBaseUrl: string | undefined = process.env.API_SERVICE_URL, asText=false) {
    if (!apiBaseUrl) {
...
export async function backendFetch(pathname: string, apiBaseUrl: string | undefined = process.env.API_SERVICE_URL, relative:boolean=false) {
    if (!apiBaseUrl) {
        throw new Error("`apiBaseUrl` cannot be null.  Please specify explicitly or set API_SERVICE_URL in .env.local")
    }
