...
export function EnhancedSearch({
    ...
    const getSuggestions = (value: string) => {
        if (value) {
            // setUrl(prefixClientRoute(`/api-proxy/service/search?keyword=${value}&limit=10`));
            setUrl(prefixClientRoute(`/api-proxy/service/search?keyword=${value}&limit=10&searchType=feature`));
        }
    };
